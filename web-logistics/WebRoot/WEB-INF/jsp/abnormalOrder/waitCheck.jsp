...
<html>
  ...
  <body>
    <table id="tt"  class="easyui-datagrid" title="待审核订单信息" style="width:100%;height:100%"
		...
		url:'bad/allBadOrder.do?state='+0,method:'post'">
		<thead>
			<tr>
	...
	//给自动生成的序号列加标题
		...
		//查询方法
		 ...
		 function reject(id){
			$('#dlgg').window('center');
			$('#dlgg').dialog('open').dialog('setTitle','驳回意见');
			$('#fm').form('clear');
			url = "bad/update.do?badorder_id="+id+"&val="+1;
		} 
		//通过
		function pass(id){
					...
					if (r){
					 //jquery的ajax操作  $.get(地址，参数，回调函数，返回值类型)
					   //  $.post(地址，参数，回调函数，返回值类型)
						$.post('bad/update.do',{badorder_id:id,val:2 },function(result){
							if (result.msg=="提交成功"){

...
<html>
  ...
  <body>
    <table id="tt"  class="easyui-datagrid" title="维修账单支付" style="width:100%;height:100%"
		pagination="true" fitColumns="true" toolbar="#tb" rownumbers="true" data-options="singleSelect:true,collapsible:true,
		url:'repair/allCarHistory.do?bill_type=1',method:'post'">
		<thead>
			<tr>
				...
				<th data-options="field:'repair_history_address',width:100,align:'center'">损坏地址</th>
	...
	<div id="dlg" class="easyui-dialog" style="width:400px;height:300px;padding:10px 20px"
		closed="true"  buttons="#dlg-buttons">
			<form id="fm" method="post">
				<div class="fitem">
					<label>维修金额</label><br>
					<input name="repair_history_repairMoney"  class="easyui-validatebox" readonly="readonly">
				</div>
				<div class="fitem">
					<label>报销金额</label><br>
					<input name="apply" class="easyui-validatebox"  required="true">
				</div>
				<div class="fitem">
					<label>收款人</label><br>
		...
		<div id="dlg-buttons">
			<a   class="easyui-linkbutton" iconCls="icon-ok" onclick="savePayee()"> 保存</a>
	...
	function formatOperation(val,row,index){
		...
		return '<input   type="button"  value="查看详情" onclick="view(\''+row.repair_history_id+'\')">|<input   type="button"  value="费用支付" onclick="payBill(\''+row.repair_history_id+'\')"></input>';
	}
	...
	//给自动生成的序号列加标题
		...
		//查询方法
		 ...
		 function view(data){
			$('#fmm').form('clear');
			$('#fmm').form('load',"repair/selectHisById.do?repair_history_id="+data);
		 ...
		 function payBill(data){
			...
			$('#dlg').window('center');
			url = "repair/savehistory.do?val="+66+"&repair_history_id="+data;
		 }
		 	//保存操作
		function savePayee(){

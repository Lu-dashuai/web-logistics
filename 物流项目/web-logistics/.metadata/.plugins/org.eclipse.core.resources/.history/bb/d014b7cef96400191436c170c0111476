...
<html>
  ...
  <body>
    <table id="tt"  class="easyui-datagrid" title="未通过订单信息" style="width:100%;height:100%"
		pagination="true" fitColumns="true" toolbar="#tb" rownumbers="true" data-options="singleSelect:true,collapsible:true,
		url:'bad/allBadOrder.do?state='+1,method:'post'">
		<thead>
			<tr>
	...
	<div id="dlg" class="easyui-dialog" style="width:400px;height:300px;left: 600px;top:300px"
		...
		<form id="ff" method="post">
				...
				<div class="fitem">
					<label>复核意见:</label>
				 <input  name="back2_reason" class="easyui-validatebox" readonly="readonly"> 
				</div>
			</form>
		</div>
		<div id="dlg-buttons">
	...
	<div id="dlgg" class="easyui-dialog" style="width:400px;height:300px;padding:10px 5px"
		...
		<form id="fm" method="post">
				<div class="fitem">
					<label>商讨后赔偿金额:</label> <br>
					  <input  name="advise_money" class="easyui-validatebox" required="true">
				</div>
				<div class="fitem">
					<label>原因:</label><br>
				 <textarea name="emp_details" style="width:200px;height:80px;" class="easyui-validatebox" required="true"></textarea>
				</div>
			</form>
		</div>
		<div id="dlgg-buttons">
	...
	function formatOperation(val,row,index){
		...
		return '<input id="tjsq"  type="button"  value="提交申请" onclick="subApply(\''+row.badorder_id+'\')">';
	...
	//给自动生成的序号列加标题
		...
		//查询方法
		 ...
		 //驳回
		 function subApply(id){
			$('#dlgg').window('center');
			$('#dlgg').dialog('open').dialog('setTitle','商讨后的意见');
			$('#fm').form('clear');

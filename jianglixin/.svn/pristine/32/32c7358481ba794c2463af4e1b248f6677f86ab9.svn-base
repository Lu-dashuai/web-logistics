$(function() {
		...
		window.location.href="/lightning3/Worker/findOneWorker.action?worker.worker_id="+worker_id;
		}else{
			if(rows.length==0){
				alert("请选择一条数据进行修改");
			}else{
...
//	                    }; 
	...
	function InitMainTable() {
		...
		var queryUrl = '/lightning3/Worker/findAllWorker.action'
		$('#table').bootstrapTable({
			url : queryUrl, //请求后台的URL（*）
			treeView: true,
	        treeId: "worker_id",
	        treeField: "worker_password",
			method : 'POST', //请求方式（*）
			...
			uniqueId : "worker_id", //每一行的唯一标识，一般为主键列
...
//				}
			...
			onDblClickRow : function(row) {
				alert("员工名字" + row.worker_name +"员工ID"+ row.worker_id);
			}
		});
	};

...
<body>
	<table id="table">
    	<div id="toolbar">
			<div class="btn-group">
				<button type="button" class="btn btn-primary" id="comeinindent">审核出库</button>
				<!-- <button type="button" class="btn btn-primary" id="arrive">订单到达</button> -->
...
function InitMainTable(){
	...
	var queryUrl = '/lightning3/Indent/comeinIndent.action'//action 地址
 ...
 * 提交未出库订单，状态改为未发车
 */
$("#comeinindent").on('click', function() {
	...
	if(rows.length>0){
		...
		$.ajax({
			url:"Indent/updateComeinIndentSituation.action",//地址
			type:"post",//提交方式
			datatype:"text",//接受
			data:{"ind":ind},
			success:function(data){
				alert(data);
				$(window).attr("location","/lightning3/before/yuhaiyang/comeinIndent.jsp");
				//location.reload();
			}
		}); 
 ...
 $("#arrive").on('click', function() {
	 var rows = $('#table').bootstrapTable("getSelections");
		if(rows.length>0){
			...
			var idid=new Array();
			for(var i=0;i<rows.length;i++){
				//indentid.push(rows[i].indent_shipper_address);
				idid.push(rows[i].indent_id);
			}
			var ind=idid.join(",");
			$.ajax({
				url:"Indent/arriveIndentSituation.action",
				type:"post",
				datatype:"json",
				data:{"idid":ind/* ,"workervalue":workervalue */},

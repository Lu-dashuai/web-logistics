  			...
  			$.ajax({
  				...
  				success:function(str){
  					...
  					for(var i=0;i<indents.length;i++){
  						...
  						var situation=indents[i].situation_name;
  						var star;
  						var end;
  						...
  						if(indents[i].time_end==null&&(situation=="待审核"|| situation=="未出库"|| situation=="未发车"|| situation=="来货入库"||situation=="订单异常")){
  							end="未发车";
  						}else if(indents[i].time_end==null&&situation=="运输中"){
  							end="未到达";
  						}else{
  							end=changeDateFormats(indents[i].time_end);	
  						}
  						var strs="<td>"+indents[i].indent_id+"</td><td>"+time+"</td><td>"+situation+"</td><td>"+star+"</td><td>"+end+"</td>"
  						$("#times").append(strs);
  					}
  				}
  			});
